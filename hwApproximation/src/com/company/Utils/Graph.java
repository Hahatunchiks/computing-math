...
public class Graph {
    ...
    public void drawDependency(ApproximationFunction function) {
        chart.getStyler().setLegendPosition(Styler.LegendPosition.InsideSE);
        chart.getStyler().setDefaultSeriesRenderStyle(XYSeries.XYSeriesRenderStyle.Line);
        ArrayList<Double> dataX = new ArrayList<>(), dataY = new ArrayList<>();
        for (double i = leftX; i < rightX; i += 0.01) {
            if (!Double.isNaN(function.getFunction().apply(i))) {
                dataX.add(i);
                dataY.add(function.getFunction().apply(i));
            }
        }
        XYSeries series = chart.addSeries(function.getFunctionName(), dataX, dataY);
